...
public class MLS_MPM_Compute : MonoBehaviour {
    ...
    struct Particle {
        ...
        public float2x2 C; // affine momentum matrix, unused in this file (force representation, includes particle tension)
        public float mass;
    ...
    private float _frameInputBuffer = 0.1f;
    private IEnumerator _activeFrameInput = null;
    ...
    private void NextFrame() {
        if (_activeFrameInput == null) {
            _activeFrameInput = NextFrameAction();
            StartCoroutine(_activeFrameInput);
        }
    }

    private IEnumerator NextFrameAction() {
        Simulate();
        yield return new WaitForSeconds(_frameInputBuffer);
        _activeFrameInput = null;
        yield return null;
    }
